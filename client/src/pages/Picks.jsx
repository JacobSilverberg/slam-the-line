...
const Picks = () => {
  ...
  const handleSubmit = (gameId) => {
    ...
    axios.post('http://localhost:3001/api/submit', { selectedData })
      .then(response => {
        ...
        {data.map(game => (
          <li key={game.gameId}>
            <div>
              ...
              <button onClick={() => handleButtonClick(game.gameId, 'home')}>Home Spread: {game.spread}</button>
              <button onClick={() => handleButtonClick(game.gameId, 'away')}>Away Spread: {game.spread}</button>
              <input 
                type="number" 
              ...
              />
              <button onClick={() => handleSubmit(game.gameId)}>Submit Wager</button>
            </div>
          </li>
        ))}
      </ul>
